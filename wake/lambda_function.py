...
_cfg = Config(retries={"max_attempts": 3, "mode": "standard"}, region_name=REGION)
...
<body>
  <div class="card">
    ...
    <p class="muted" style="margin-top:8px">A cold start may take 10–120 seconds.</p>
    <p style="margin:16px 0 8px">Progress: <span class="pill">{secs_elapsed}s / ~{secs_total}s</span></p>
    ...
    <p class="muted" style="margin-top:16px;font-size:14px">This page will auto-refresh.</p>
  </div>
<script>
setTimeout(function(){{window.location.replace("{html.escape(retry_url)}");}},3000);
</script>
</body>"""
...
def _get_public_ip_from_running_task():
    ...
    for att in attachments:
        if att.get("type") == "ElasticNetworkInterface":
            details = {d["name"]: d["value"] for d in att.get("details", []) if "name" in d and "value" in d}
